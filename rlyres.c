...
{
    FILE *fp;
    fp = fopen("G:\\myproject\\alltrains.dat","rb");
    if(fp == NULL)
    {
        train alltrains[10]={
        ...
        {"11077","PUNE","JAMMU",1200,2390},
        {"546","BPL","DEL",3500,2240},
        {"12854","BPL","DRG",2100,3500},
        {"11016","GRP","LTT",2390,3400},
        {"22192","JPL","IND",2300,3430},
        {"11057","AMT","LTT",2120,3000},
        {"12061","HBJ","JPL",2000,3000}
        };
        fp = fopen("G:\\myproject\\alltrains.dat","wb");
...
void view_trains()
{
    clrscr();
    int i;
    FILE *fp=fopen("G:\\myproject\\alltrains.dat","rb");
    train tr;
    if(fp==NULL)
    {
        return ;
...
{
    FILE *fp=fopen("G:\\myproject\\allbookings.dat","rb");
        if(fp==NULL)
        return 0;
        passenger pr;
        int count=0;
...
int book_ticket(passenger p)
{
    ...
    p.ticketno=count;
    FILE *fp=fopen("G:\\myproject\\allbookings.dat","ab");
...
{
    FILE *fp1=fopen("G:\\myproject\\allbookings.dat","rb+");
    if(fp1==NULL)
    {
        textcolor(LIGHTRED);
        printf("\n NO bookings done yet");
    ...
    FILE *fp2=fopen("G:\\myproject\\temp.dat","wb+");
    passenger pr;
    int found=0;
    ...
    if(found==0)
    {
        remove("G:\\myproject\\temp.dat");
    }
    else{
        remove("G:\\myproject\\allbookings.dat");
        rename("G:\\myproject\\temp.dat","G:\\myproject\\allbookings.dat");
    }
    return found;
      ...
      while(fread(&pr,sizeof(pr),1,fp1)==1)
{
  ...
  void view_booking(char* trainno)
  {
      clrscr();
      FILE *fp;
      fp=fopen("G:\\myproject\\allbookings.dat","rb");
      if(fp==NULL)
      {
          textcolor(LIGHTRED);
          printf("\nNo booking done yet");
